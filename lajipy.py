...
class bizhi(object):
    ...
    def guolv2(self,abc):
        shuju=[]
        patt=re.compile(r'<ul id="showImg" class="clearfix" style="left:-0px;width:1752px;"><li class="show1 cur ">(.*?)</li></ul>			</div>',re.S)
        itmes=patt.findall(abc)
        for i in itmes:
            shuju.append(i)
    ...
    def save(self,abc,mu):
        ...
        for i,j in enumerate(abc):
            res=requests.get(j)
            bian=res.contentimport requests
import re
import os
class bizhi(object):
    def qinqiu(self,page):
